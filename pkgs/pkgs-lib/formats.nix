  ...
  */
  ...
  jsonWithRuntimeSubstitution = {}: rec {
    ...
    generate = filename: value: let
      ...
      substitutionToJq = index: { attrpath, source, ... }:
        lib.concatMapStrings (attr: "." + (if lib.isString attr then builtins.toJSON attr else "[${toString attr}]")) attrpath
        + " =  " + (if source ? envVar then "$ENV.${source.envVar}" else "$file${toString index}")
      ;
      jqScript = lib.concatStringsSep "\n |" (lib.imap0 substitutionToJq substitutions);
